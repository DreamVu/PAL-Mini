...
if [ "$ARCH" = "x86_64" ]; then
	echo "set(PAL_LIBRARY" `pwd`/../lib/x86_64/libPAL_MINI.so ")" >> ../dreamvu_pal_navigation/CMakeLists.txt
else
	echo "set(PAL_LIBRARY" `pwd`/../lib/arm64/libPAL_MINI.so ")" >> ../dreamvu_pal_navigation/CMakeLists.txt
fi
