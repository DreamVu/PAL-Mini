...
float GetDepthValue(cv::Mat depth, PAL::BoundingBox Box)
{
	std::vector<float> depth_values;
	
	
	cv::Point p1 = cv::Point(Box.x1%depth.cols, Box.y1%depth.rows);
	cv::Point p2 = cv::Point(Box.x2%depth.cols,Box.y2%depth.rows);

	
	cv::Rect roi = cv::Rect(p1,p2);
	cv::Mat matBox_cm = depth(roi).clone();
	

        
	int frameSize = matBox_cm.rows * matBox_cm.cols;

	matBox_cm = matBox_cm;
	cv::Mat fmatBox_m = matBox_cm/100;
        //cv::Mat fmatBox_m = 3.37*matBox_cm/100;
	cv::Mat imatBox_m;

	fmatBox_m.convertTo(imatBox_m, CV_8UC1);
	//cout<<imatBox_m<<endl;
	unsigned char* pimatBox_m = (unsigned char*)imatBox_m.data;
...
int main(int argc, char *argv[])
{
	...
	while(key != 27)
		...
		std::vector<float> DepthValues(num_of_persons, 100.0f);
		char text[128];

		for(int i =0; i<num_of_persons; i++)
		{

		    DepthValues[i] =  GetDepthValue(depth, Boxes[i]);
		    
	 	    sprintf(text, "Depth:%.0fm", DepthValues[i]);
		    ...
		    {
			cv::rectangle(rgb,Point(Boxes[i].x1, Boxes[i].y1), Point(Boxes[i].x2, Boxes[i].y2), cv::Scalar(0,0,255), 5);

			setLabel(rgb, text, Point(Boxes[i].x1, Boxes[i].y1), CV_RGB(255,0,0));
